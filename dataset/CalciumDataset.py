...
from dataset.extractors import PatchExtractor, SliceExtractor
from utils.util import random_item
...
class Dataset:
    ...
    def __init__(self, subset, metadata, config, mode='images_only', kernels='all'):
        ...
        # image_files = glob("{}/*.mhd".format(self.imagedir))
        # n_scans = len(image_files)
        # self.uids = []
        # for f in image_files:
        #     self.uids.append( path.basename(f)[:-4] )
    ...
    def extract_slices(self):
        ...
        if path.exists(cachefile):
            ...
            for uid in tqdm(self.uids, desc='Processing images'):
                overlay = read_image(path.join(self.overlaydir, uid + '.mhd'), only_data=True)
    ...
    def make_patch_extractor(self, uid):
        image_filename = path.join(self.imagedir, uid + '.mhd')
    ...
    def make_overlay_slice_extractor(self, uid):
        overlay_filename = path.join(self.overlaydir, uid + '.mhd')
...
class MutableSubsetOfSlices_nomask:
    """A subset of a dataset which can slide over the entire dataset"""
    def __init__(self, dataset, size ):
        self.dataset = dataset
        self.size = size
    ...
    def swap_images(self, n_images):
        ...
        # Combine individial lists of slices into a big lists
        self.slices = []
        for uid in self.uids:
            # slice = 0
            image = read_image(path.join(self.dataset.imagedir, uid + '.mhd'), only_data=True)
            for z in range(image.shape[0]):
                self.slices.append((uid, z))
                # slice += 1

        print(' > Removed {} and added {} images to subset of dataset "{}"'.format(
            len(removed_uids), len(added_uids), self.dataset.name))

        print(' > Subset now contains {} images with {}  slices'.format(
            len(self.uids), len(self.slices)))


def balanced_minibatch_of_cropped_slices_iterator(subset, n_minibatches, axis=0, Centering=True):
    ...
    half_slice_width = subset.dataset.config['slice_size_voxels'] // 2
    ...
    for n_processed in range(n_minibatches):
        ...
        for i in range(minibatch_size):
            if i < minibatch_size // 2:
                ...
                # # Only extract patches at high intensity region.
            ...
            for axis_index, axis_id in enumerate(axes):
                ...
                slices[i, axis_index, :, :] = e.extract_cropped_ortho_slice(axis_id, p, half_padding + half_slice_width, Centering=Centering)

                e = subset.overlay_slice_extractors[uid]
                overlay_cropped = e.extract_cropped_ortho_slice(axis_id, p, half_slice_width, pad_val=0, Centering=Centering)
        ...
        yield torch.tensor(slices), torch.tensor(labels)

def get_a_batch_of_cropped_slices_iterator(subset, n_minibatches, T=130, axis=0, Centering=True, withGT=False):
    """ Iterates over a MutableSubsetOfSlices in minibatches """
    slice_xy = subset.dataset.config['slice_size_voxels'] + subset.dataset.config['slice_padding_voxels']
    label_xy = subset.dataset.config['slice_size_voxels']
    minibatch_size = subset.dataset.config['minibatch_size']
    ...
    if withGT:
        all_slices = subset.slices[0] + subset.slices[1]
    else:
        all_slices = subset.slices
    ...
    for n_processed in range(n_minibatches):
        ...
        for i in range(minibatch_size):
            ...
            # The first bit of the minibatch should be from locations with high intensities
            if i < minibatch_size // 2:
                indices = np.transpose(np.where(image_slice >= T))
                y, x = tuple(indices[np.random.randint(0, indices.shape[0]), :])
            else:
                y = np.random.randint(0, slice_shape[0])
                x = np.random.randint(0, slice_shape[1])
        ...
        # normalize
        slices = (np.clip(slices, a_min=-1000, a_max=3000) - 130) / 1130.0
        if withGT:# with ground truth for source domain
            yield [torch.tensor(slices), torch.tensor(labels)]
        else:# without ground truth for target domain
            yield torch.tensor(slices)
...
class MutableSubsetOfPositiveVoxelsWithMask:
    ...
    def swap_images(self, n_images):
        ...
        for uid in added_uids:
            overlay = read_image(path.join(self.dataset.overlaydir, uid + '.mhd'), only_data=True)
            ...
            mask = read_image(path.join(self.dataset.maskdir, uid + '.mhd'), only_data=True)
...
def balanced_minibatch_iterator_with_mask(subset, n_minibatches, p_positive=0.5, p_negative_mask=0.4):
    ...
    for n_processed in range(n_minibatches):
        ...
        patches = np.zeros(shape=(minibatch_size, 3, patch_xy, patch_xy), dtype=PatchExtractor.dtype)
        ...
        for i in range(minibatch_size):
            ...
            patches[i, :, :, :] = subset.extractors[uid].extract_orthogonal(coords)

        # yield patches, labels
        yield torch.tensor(patches), torch.tensor( labels, dtype=torch.long )
