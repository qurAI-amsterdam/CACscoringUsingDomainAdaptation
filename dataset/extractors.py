...
class SliceExtractor:
    ...
    @staticmethod
    def compute_crop(x, axis_length, half_size, Centering=True):
        x_from = x - half_size
        x_to = x + half_size + 1 if Centering else x + half_size
    ...
    @staticmethod
    def get_crop(image, z, yx, half_size, pad_val, Centering=True):
        y_from, y_to, y_padding_left, y_padding_right = SliceExtractor.compute_crop(yx[0], image.shape[1], half_size, Centering=Centering)
        x_from, x_to, x_padding_left, x_padding_right = SliceExtractor.compute_crop(yx[1], image.shape[2], half_size, Centering=Centering)
        ...
        if y_padding_left == 0 and y_padding_right == 0 and x_padding_left == 0 and x_padding_right == 0:
            ...
            if Centering:
                ...
                cropped = np.empty(shape=(2 * half_size, 2 * half_size), dtype=SliceExtractor.dtype)
    ...
    def extract_cropped_ortho_slice(self, axis, zyx, half_size, Centering=True, pad_val=-1000):
        ...
        slice = SliceExtractor.get_crop(image, z, yx, half_size, pad_val, Centering=Centering)
